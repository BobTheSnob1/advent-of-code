...
class Equation:
    def __init__(self, equation):
        ...
        self.operands = [
            int(_) for _ in equation.split(':')[1].strip().split(' ')
        ]
...
def solvable(target, current_result, operands):
    ...
    if current_result > target or len(operands) == 0:
        ...
        return (solvable(target, current_result + operands[0], operands[1:]) or
                solvable(target, current_result * operands[0], operands[1:]) or
                solvable(
                    target,
                    concat_numbers(current_result, operands[0]),
                    operands[1:]
                )
                )
